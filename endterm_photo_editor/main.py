...
class PyPhotoEditor:
    ...
    def delete_current_image(self):
        ...
        if not mb.askokcancel("Delete image", "Уверен брат?\nПотом потеряется."):
            return
    ...
    def _close(self, event=None):
        if self.unsaved_images():
            if not mb.askyesno("Оппааа", "Не сохранилось, уверен?"):
                return
    ...
    def show_tips(self):
        tips = [
            "Добро пожаловать в Py Photo Editor! Это простой фоторедактор на Python с использованием библиотеки Tkinter. Вы можете открыть изображения, редактировать их и сохранить изменения.",
            "Открытие изображений: Для открытия изображений выберите пункт меню 'File -> Open' или используйте сочетание клавиш. Выберите одно или несколько изображений из файловой системы и нажмите 'Открыть'.",
            "Редактирование изображений: После открытия изображения вы можете использовать меню 'Edit' для редактирования. Выберите функцию, такую как поворот, изменение размера, обрезка или преобразование цветности.",
            "Сохранение изменений: Чтобы сохранить изменения, выберите пункт меню 'File -> Save' или 'File -> Save as'. В первом случае изменения будут сохранены в исходном файле, во втором случае можно выбрать новое имя файла.",
            "История редактирования: Вы можете отменить или повторить предыдущие действия с изображением, используя соответствующие пункты меню или горячие клавиши.",
            "Сохранение конфигурации: Последние открытые изображения сохраняются автоматически при выходе из программы и загружаются при следующем запуске.",
            "Меню 'Open Recent': Вы можете быстро выбрать из последних открытых изображений, используя пункт меню 'File -> Open Recent'."
        ]
        mb.showinfo("Подсказки", "\n\n".join(tips))
